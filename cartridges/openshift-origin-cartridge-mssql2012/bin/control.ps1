...
$MSSQL2012_PID_FILE = $env:OPENSHIFT_MSSQL2012_DIR+"\run\mssql.pid"
$env:MSSQL2012_PID_FILE = $MSSQL2012_PID_FILE

#Start the software the cartridge controls
function start-cartridge
{
    if (process_running "powershell" $MSSQL2012_PID_FILE)
    {
        Write-Host "Cartridge already running"
    }
    else
    {
        Write-Host "Starting MSSQL2012 Cartridge"
        "Starting" > $MSSQL2012_PID_FILE

        $logDir = (Join-Path $env:OPENSHIFT_MSSQL2012_DIR 'log')
        New-Item -path $logDir -type directory -Force | Out-Null

        $job = Start-Process powershell -argument "$env:OPENSHIFT_MSSQL2012_DIR\bin\start.bat  1>> ${logDir}\stdout.log 2>> ${logDir}\stderr.log" -passthru -windowstyle hidden
        $job.Id > $MSSQL2012_PID_FILE

        $currentDir = split-path $SCRIPT:MyInvocation.MyCommand.Path -parent
        $instanceName = "Instance${env:OPENSHIFT_MSSQL2012_DB_PORT}"
        ...
        client_result ""
        client_result "Microsoft SQL Server 2012 database added.  Please make note of these credentials:"
        client_result ""
        client_result "     sa password: ${password}"
        client_result "   database name: ${env:OPENSHIFT_APP_NAME}"
        client_result ""

        client_result "Connection URL: mssql://`$OPENSHIFT_MSSQL2012_DB_HOST:`$OPENSHIFT_MSSQL2012_DB_PORT/"
        client_result ""
    }
...
function stop-cartridge
{
    ...
    {
        Write-Host "Stopping"
        $jobid = [int](Get-Content $MSSQL2012_PID_FILE)
        Remove-Item $MSSQL2012_PID_FILE
